...
void NetlinkWatcher::requestDump()
{
    ...
    if (sendto(m_fd, &req, req.hdr.nlmsg_len, 0, reinterpret_cast<struct sockaddr *>(&dst), sizeof(dst)) < 0) {
...
void NetlinkWatcher::processPendingMessages(const LinkChangeCallback &onChange, const LinkRemovedCallback &onRemoved)
{
    char buf[8192];
    while (true) {
        ssize_t n = recv(m_fd, buf, sizeof(buf), MSG_DONTWAIT);
        if (n < 0)
            break; // EAGAIN/EWOULDBLOCK: no more messages right now
        if (n == 0)
            break;
